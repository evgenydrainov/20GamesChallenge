...
void Game::Frame() {
	...
	// :update
	{
		...
		for (int i = 0; i < enemy_count; i++) {
			Enemy* e = &enemies[i];
			if (e->type == 10) {
				// e->angle = point_direction(e->x, e->y, player.x, player.y);
				// e->hsp += lengthdir_x(0.3f, e->angle) * delta;
				// e->vsp += lengthdir_y(0.3f, e->angle) * delta;
				// if (length(e->hsp, e->vsp) > 10.0f) {
				// 	float dir = point_direction(0.0f, 0.0f, e->hsp, e->vsp);
				// 	e->hsp = lengthdir_x(10.0f, dir);
				// 	e->vsp = lengthdir_y(10.0f, dir);
				// }
				...
				if (Player* p = find_closest(&player, 1, e->x, e->y, &rel_x, &rel_y, &dist)) {
					if (dist > 1000.0f && e->catch_up_timer == 0.0f) {
						e->catch_up_timer = 60.0f;
					}
					...
					spd += 0.2f;
					float max_spd = (e->catch_up_timer > 0.0f) ? 12.0f : 8.0f;
